...
| --- | ---: | --- | ---: | --- | --- | --- | --- | --- |
| Dynamic TMoE | `0` | 关闭 | 10 | 关闭 | `32/32/32/32` | `2/2/2/2` | `8/4/2/1` | `32/32/16/16` |
...
| TimeMixer | `4`（ILI `2`） | 开启 | 5 | 关闭 | `256/128/64/32` | `64/32/16/8` | `32/16/8/4` | `32/32/16/16` |
...
当前 `--batch-size` 映射按 horizon 全局应用到本次任务中的所有模型和数据集。要使用上述矩阵，不要把十个模型与全部数据集放进同一个笛卡尔积任务；至少按“单模型 + 单数据集分组”拆分，追求最优时使用“单模型 + 单数据集”。
