...
统一入口 `train_openi.py` 支持 Dynamic TMoE、DLinear、FEDformer、FITS、PatchTST、RAFT、ST-MTM、TFPS、TimeMixer 和 TimesNet。

上述模型均支持 ETTh1、ETTh2、ETTm1、ETTm2、Electricity、Exchange、ILI、Traffic 和 Weather。TimeMixer 的 Exchange 使用通用 `Dataset_Custom` 数据加载器和 8 通道配置，ILI 使用 `36/18` 的输入/标签长度。
...
训练加速还可使用 `--use-amp true|false`、`--patience`、`--pin-memory true|false`、`--persistent-workers true|false`、`--prefetch-factor`、`--cudnn-benchmark true|false` 和 `--cpu-threads`。平台使用 Key/Value 输入时，四个布尔参数的 Value 均填写 `true` 或 `false`。V100 32GB 的分模型建议见 [V100 32GB Baseline 训练加速参数矩阵](v100-32gb-training-acceleration-matrix.md)。
